...
export default function Workforce() {
  const { data, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => apiGet<WorkforceAlert[]>('/workforce/alerts'),
  });
  ...
  const columns = [
    {
      key: 'created_at',
      header: 'Timestamp',
      render: (row: WorkforceAlert) => new Date(row.created_at).toLocaleString('pl-PL'),
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-neutral-500">{data?.length ?? 0} workforce alerts</p>
